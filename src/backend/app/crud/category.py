...
class CRUDCategory(CRUDBase):
    ...
    @staticmethod
    async def _select_categories_expenses(session: AsyncSession, where_stmt: bool):
        """Select the list of expenses by categories for specified period."""
        categories_expenses = await session.execute(
            select(Category.name, func.sum(Expense.amount).label("category_expense"))
            .join(Expense)
            .where(where_stmt)
            .group_by(Category.name)
            .order_by(desc("category_expense"))
        )
    ...
    async def get_this_month_expenses_by_categories(self, session: AsyncSession):
