...
async def add_category(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ENTER_CATEGORY_NAME, reply_markup=ReplyKeyboardRemove()
    )
    return CONFIRMATION
...
async def _category_confirmation(
    ...
    try:
        ...
        logging.info(ADDED_CATEGORY_LOG.format(get_user_info(update), category_name))
        ...
        logging.warning(
            CATEGORY_ALREADY_EXISTS_LOG.format(get_user_info(update), category_name)
        )
        await update.message.reply_html(CATEGORY_ALREADY_EXISTS.format(category_name))
        return CONFIRMATION
    except ValueError:
        logging.warning(
            CATEGORY_NAME_TOO_LONG_LOG.format(get_user_info(update), category_name)
        )
...
add_category_handler = ConversationHandler(
    ...
    states={
        CONFIRMATION: [
            MessageHandler(filters.TEXT & ~filters.COMMAND, _category_confirmation)
        ]
    },
